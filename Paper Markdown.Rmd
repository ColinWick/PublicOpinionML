...
knitr::opts_chunk$set(echo = FALSE,message = FALSE,warning=FALSE,cache=T,fig.align = "center")
library(tidyverse)
library(scales)
library(igraph)
library(rpart)
...
#### Ideological Variable Construction
...
align=TRUE, dep.var.labels=c("Position","Extremity","Consistency"),summary.stat = c("n"),type = "html",single.row = T,no.space = T,omit = c("STATE"),summary = T)
```
...
Finally, comparing $R^2$ across each of these models, we see the step regression essentially splits the difference in terms of model fit. The trade-off is strict econometric rigor at the benefit of uncovering identity interactions driving ideological position and structure in the electorate. 
